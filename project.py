...
VANTAGE_API_KEY = st.secrets["VANTAGE_API_KEY"]
...
def get_selected_week_range(selected_date):
    ...
    target_date = selected_date if isinstance(selected_date, tuple) else selected_date
...
def get_alpha_vantage_news(start_dt, end_dt, api_key=VANTAGE_API_KEY):
...
# def extract_live_justification_gemini(url):
...
#         if len(text) < 100:
#             return "Insufficient content inside the source article to extract a reliable justification."
            
#         prompt = f"""
#         You are an elite institutional market research analyst.
#         Review the following raw text scraped from a financial news article:
        
#         {text[:4000]}
        
#         Provide a concise, sophisticated 2-to-3 sentence justification summarizing the underlying micro/macroeconomic catalyst driving this story. Keep it highly objective, factual, and clear.
#         """
#         chat = client.chats.create(model='gemini-3.5-flash-lite')
#         response = chat.send_message(prompt)
#         return response.text.strip()
#     except Exception as e:
#         return f"Live AI text synthesis unavailable. (Error: {str(e)})"

def find_common_and_recommend_gemini(yahoo, investing, av):
    ...
    for idx, art in enumerate(all_articles):
        article_list_str += f"ID: {idx} | Source: {art['source']} | Title: {art['title']} | Date: {art['published_date']} | URL: {art['link']}\n"

    class NewsCluster(BaseModel):
        primary_headline: str
        recommendation_action: str
        sources_reporting: list[str]
        all_matched_links: list[str]
    ...
    prompt = f"""
    You are an AI data architect grading multi-source financial feeds.
    Analyze the following list of news articles pulled from Yahoo Finance, Investing.com, and Alpha Vantage:
    
    {article_list_str}
    
    Tasks:
    1. Identify articles that are reporting on the similar same news story or market event (Semantic Cross-Referencing).
    2. Only keep a cluster if it contains unique articles from AT LEAST TWO (2) different sources. Ignore single-source stories.
    ...
    7. Determine the 'recommendation_action' for the headline. It must strictly be one of these three values:
       ...
       - "SELL / CAUTION"
       - "HOLD / MONITOR"
    8. Extract the article published dates in 'published_date'.
    ...
    try:
        # 1. Pass the config during chat initialization
        chat = client.chats.create(
            model='gemini-3.5-flash-lite',
            config=types.GenerateContentConfig(
                response_mime_type="application/json",
                response_schema=MarketReportResponse,
                temperature=0.1
            )
        )

        # 2. Send the message through the initialized chat
        response = chat.send_message(prompt)
        
        structured_data = MarketReportResponse.model_validate_json(response.text)
        
        matched_results = []
        for cluster in structured_data.clusters:
            # with st.spinner(f"Extracting AI Insights for: {cluster.primary_headline[:50]}..."):
            #     justification = extract_live_justification_gemini(cluster.representative_url)
                
            matched_results.append({
                "Primary Headline": cluster.primary_headline,
                "Published Date (MYT)": cluster.published_date,
                "Recommendation Action": cluster.recommendation_action,
                "Live Justification": cluster.justification,
                "Sources Reporting": ", ".join(cluster.sources_reporting),
                "All Matched Links": " | ".join(cluster.all_matched_links)
            })
            
        return matched_results
    except Exception as e:
        st.error(f"Failed to generate automated Gemini clusters: {str(e)}")
        return []
...
with st.spinner("Executing background feeds synchronization & Gemini AI semantic cross-matching..."):
    ...
    new_results = find_common_and_recommend_gemini(yahoo_headlines, investing_headlines, av_headlines)
    ...
    if new_results:
        ...
        if not df_master.empty:
            df_master = pd.concat([df_master, df_new]).drop_duplicates(subset=["Primary Headline"], keep="first")
        else:
            df_master = df_new
...
if not filtered_df.empty:
    ...
    with col_head2:
        ...
        st.download_button(
            ...
            data=csv_bytes,
            file_name=f"financial_market_weekly_report_{target_date_str.replace(' ', '_')}.csv",
            mime="text/csv",
